...
public class TabOperation {
    ...
    public int[][] rotate(int[][] brickMatrix) {
        int[][] rotatedMatrix = new int[brickMatrix[0].length][brickMatrix.length];
        for (int i = 0; i < brickMatrix.length; i++) {
            for (int j = 0; j < brickMatrix[i].length; j++) {
                rotatedMatrix[j][brickMatrix.length - 1 - i] = brickMatrix[i][j];
            }
        }
